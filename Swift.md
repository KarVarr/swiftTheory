...
## Основы
$${\color{red}Можно}$$Можно обозначать переменные или константы в один ряд через запятую со значениями. Так же можно обозначать имена переменных в один ряд через запятую без значения, только указав тип в конце. 
