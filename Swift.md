...
* Не используйте значки для идентификации системных функций аутентификации. Когда люди видят значки, похожие на системные значки Touch ID (отпечаток большого пальца) и Face ID, они думают, что должны пройти аутентификацию.
...
* Внедряйте контекстные меню последовательно. Если вы предоставляете контекстные меню для элементов в одних местах, но не в других, люди не будут знать, где они могут использовать эту функцию, и могут подумать, что в вашем приложении есть проблема.
...
* Ограничьте количество сегментов, чтобы улучшить удобство использования.
* Старайтесь, чтобы размер содержимого сегментов был одинаковым.
* Избегайте смешивания текста и изображений в сегментированном элементе управления.
* Правильно размещайте содержимое в пользовательском сегментированном элементе управления. Если вы измените внешний вид фона сегментированного элемента управления, убедитесь, что содержимое по-прежнему выглядит хорошо и не смещено.


### *Переключатели*
* Используйте переключатели только в строках таблицы. Переключатели предназначены для использования в таблицах, например, в списке параметров, которые можно включать и выключать. Если вам нужна аналогичная функциональность на панели инструментов или навигационной панели, используйте вместо нее кнопку и предоставьте два разных значка, передающих состояния.
* Избегайте добавления меток для описания значений переключателей. Переключатели либо включены, либо выключены. Предоставление меток, описывающих эти состояния, является избыточным и загромождает интерфейс.
...
Глобальные или статические переменные хранятся в области данных, а не в стеке.
    ...
    static var globalPoint = Point(x: 10, y: 20)

Использование замыканий (closures):
Если тип значения захватывается замыканием, он может быть размещен в куче.
    ...
    func capturePoint() -> () -> Point {
        let point = Point(x: 10, y: 20)
        return { point }
    }

Использование коллекций:
Swift коллекции, такие как массивы, словари и множества, хранят свои элементы в куче.
    ...
    var points = [Point(x: 0, y: 0), Point(x: 10, y: 10)]
...
Структуры и enumы не могут через свои методы изменять свои же свойства из-за того, что это тип значения, только если перед методом поставить слово **mutating**. Класс же может изменять в методе свои же свойства. В языке Swift структуры являются типами значений, а это означает, что каждое их копирование создает новую копию данных. Когда вы работаете с экземпляром структуры, вы фактически работаете с копией этих данных. Из-за этого изменения данных в структуре могут потребовать изменения самой структуры.
...
В расширениях по умолчанию нельзя использовать свойства хранения(сторед проперти). 
...
Если для стуктуры объявить init в расширении то дефолтный init не пропадет и в структуре уже будет два init. Если же написать init в структуре то дефолный init пропадет.
...
NotificationCenter — это объект, который управляет механизмом уведомлений **внутри самого приложения**. Он позволяет объектам отправлять сообщения (уведомления) другим объектам без необходимости связывать их напрямую. Это помогает реализовать общение между различными частями приложения, не создавая жестких связей между объектами (что облегчает поддержку и тестирование кода).

**Как это работает?**
NotificationCenter используется для отправки и получения сообщений (уведомлений). Когда что-то происходит (например, данные загружены, пользователь выполнил действие), один объект может отправить уведомление, а другие объекты могут подписаться на него и реагировать.

**Пример использования NotificationCenter:**
Отправка уведомления: Один объект отправляет уведомление о событии:

    NotificationCenter.default.post(name: Notification.Name("DataDidLoad"), object: nil)
    
Подписка на уведомление: Другой объект подписывается на это уведомление, чтобы выполнять действия, когда оно произойдет:

    NotificationCenter.default.addObserver(self, selector: #selector(handleDataLoaded), name: Notification.Name("DataDidLoad"), object: nil)

    @objc func handleDataLoaded(notification: Notification) {
        print("Данные загружены!")
    }
    
Удаление наблюдателя: Когда подписка на уведомления больше не нужна, нужно удалить наблюдателя, чтобы избежать утечек памяти:

    NotificationCenter.default.removeObserver(self, name: Notification.Name("DataDidLoad"), object: nil)

**Когда используется NotificationCenter?**
* Когда нужно уведомлять несколько частей приложения об одном и том же событии (например, данные загружены, пользователь выполнил вход в систему и т.д.).
* Когда нет необходимости связывать объекты напрямую, что позволяет улучшить модульность и гибкость кода.
  
**Преимущества:**
* Децентрализация: объекты могут взаимодействовать друг с другом без прямых ссылок, что уменьшает связность кода.
* Гибкость: один объект может уведомлять несколько объектов о каком-то событии.
  
> Пример использования NotificationCenter в реальной жизни:
> Представь приложение для обмена сообщениями. Когда приходит новое сообщение, уведомление отправляется через NotificationCenter, и все части интерфейса, которые должны отреагировать (например, обновить список чатов), получают уведомление и выполняют свои действия.

**Когда лучше избегать NotificationCenter?**
Если в проекте есть много уведомлений, их количество может стать трудно управлять, и код может стать сложно поддерживать.
В новых проектах иногда предпочитают использовать современные библиотеки, например Combine, для более декларативного управления событиями.
...
* UISplitViewController: Разделения интерфейса на два контроллера для iPad.
...
* Простой поиск выполняется за линейное время. **O(n)**
* Бинарный поиск работает быстрее — за логарифмическое время: **O(log n)**
* Поиск данных в хеш-таблице выполняется за постоянное время. **О(1)**
...
Хеш-функция представляет собой функцию, которая получает строку4 и возвращает число:
...
Хеш-таблица состоит из ключей и значений. В хеше book имена продуктов являются ключами, а цены — значениями. Хеш-таблица связывает ключи со значениями. Они также известны под другими названиями: «ассоциативные массивы, «словари», «отображения», «хеш-карты» или просто «хеши». Хеш-таблицы исключительно быстро работают! **О(1)**

## Поиск в ширину Графы

**«Что такое граф?»**

Графы состоят из узлов и ребер. Узел может быть напрямую соединен с несколькими другими узлами. Эти узлы называются соседями. Графы используются для моделирования связей между разными объектами.
...
Нужно найти в сети хотя бы одного продавца манго. Если Алиса не продает манго, то в список добавляются ее друзья. Это означает, что со временем вы проверите всех ее друзей, а потом их друзей и т.д. С этим алгоритмом поиск рано или поздно пройдет по всей сети, пока вы все-таки не наткнетесь на продавца манго. Такой алгоритм и называется **поиском в ширину.**
У Ануджа, Пегги, Тома и Джонни соседей нет. Линии со стрелками указывают на них, но не существует стрелок от них к другим узлам. Такой граф называется **направленным** — отношения действуют только в одну сторону. В **ненаправленном** графе стрелок нет, и каждый из узлов является соседом по отношению друг к другу.
...
> Hапомню: ребром называется соединительная линия или линия со стрелкой, ведущая от одного человека к другому.

> Генеалогическое древо — тоже граф, потому что в нем есть узлы (люди) и ребра. Ребра указывают на родителей человека. Естественно, все ребра направлены вниз — в генеалогическом дереве ребро, указывающее вверх, не имеет смысла. Ведь ваш отец никак не может быть дедушкой вашего дедушки! Такая особая разновидность графа, в которой нет ребер, указывающих в обратном направлении, называется **деревом**.
...
Поиск в ширину "Графы" - мы использовали поиск в ширину для нахождения кратчайшего пути между двумя точками. Тогда под «кратчайшим путем» понимался путь с минимальным количеством сегментов. С другой стороны, в алгоритме Дейкстры каждому сегменту присваивается число **(вес)**, а алгоритм Дейкстры находит путь с наименьшим суммарным весом. 
Когда вы работаете с алгоритмом Дейкстры, с каждым ребром графа связывается число, называемое весом.

> Граф с весами называется взвешенным графом. Граф без весов называется невзвешенным графом. Для вычисления кратчайшего пути в невзвешенном графе используется поиск в ширину. Кратчайшие пути во взвешенном графе вычисляются по алгоритму Дейкстры.
...
Жадный алгоритм прост: на каждом шаге он выбирает оптимальный вариант.В технической терминологии: на каждом шаге выбирается локально-оптимальное решение, а в итоге вы получаете глобально-оптимальное решение. Хотите верьте, хотите нет, но этот простой алгоритм успешно находит оптимальное решение задачи составления расписания!
...
* ***Как Any реализован под капотом?***
    ...
    Any в Swift – это тип, который может содержать значение любого другого типа. Он используется для обеспечения универсальности и работы с кодом, не зависящим от конкретных типов.
    
    **Реализация Any под капотом:**
    
    * Existential type: Any – это экзистенциальный тип, что означает, что он не имеет фиксированного размера или структуры. Он представляет собой "сущность", а не конкретное значение.
    * Объектная оболочка: Any реализован как объектная оболочка, содержащая ссылку на объект и его тип.
    * Boxing и unboxing: При присвоении значения типу Any происходит его "упаковка" (boxing) – преобразование в объект-оболочку. При извлечении значения из Any происходит "распаковка" (unboxing) – преобразование обратно в исходный тип.
    * Протокол AnyObject: Any использует протокол AnyObject, который является базовым протоколом для всех объектов Swift.
    * Динамическая типизация: Any использует динамическую типизацию, что означает, что тип значения не известен заранее и определяется во время выполнения.
      
    **Преимущества Any:**
    * Универсальность: Any позволяет работать с кодом, не зависящим от конкретных типов.
    * Расширяемость: Any позволяет легко добавлять новые типы в код.
      
    **Безопасность:**
    * Any помогает избежать ошибок, связанных с несовместимостью типов.
  
    **Недостатки Any:**
    * Низкая производительность: Any может быть менее производительным, чем использование конкретных типов.
    * Сложность отладки: Any может затруднить отладку кода, так как тип значения не известен заранее.

* ***Расскажи про type erasure, opaque types***

    **Type Erasure:**
    * Определение: Type erasure – это техника компиляции, которая скрывает конкретные типы данных, оставляя только их общие характеристики.
    * Цель: Type erasure используется для обеспечения универсальности и возможности работы с кодом, не зависящим от конкретных типов.
    * Пример: Array<Any> может содержать элементы любого типа, и код, работающий с этим массивом, не зависит от конкретных типов элементов.
      
    **Opaque Types:**
    * Определение: Opaque types – это типы, которые скрывают свою реализацию от пользователя.
    * Цель: Opaque types используются для обеспечения абстракции и инкапсуляции.
    * Пример: some P – это opaque type, который представляет собой значение, соответствующее протоколу P, но его конкретный тип не известен.


* ***Инициализаторы***

    * Инициализатор (конструктор) — это специальный метод, выполняющий подготовительные действия при создании экземпляра объектного типа данных. Инициализатор срабатывает при создании экземпляра, а при его удалении вызывается деинициализатор. Инициализатор выполняет         установку начальных значений хранимых свойств и различных настроек, которые нужны для использования экземпляра.
...
* ***Что такое перегрузка функций***
    ...
    * Swift позволяет перегружать функции (overloading), то есть в одной и той же области видимости создавать функции с одинаковыми имeнами. Различия функций должны заключаться в их функциональных типах (входных и выходных параметрах, разное количество параметров,           разные типы параметров или и то, и другое).
 
* ***Зачем нужно ключевое слово where?***

    * where — это способ указать дополнительные ограничения для универсальных типов, функций, связанных типов или операторов потока управления . Вы можете использовать ключевое слово where, за которым следует выражение, чтобы потребовать, чтобы                         
      универсальный параметр соответствовал протоколу, наследовался от класса или был равен определенному типу.
...
* ***Зачем нужно ключевое слово inout?***
    ...
    * В функции нельзя изменять внешнюю переменную, ведь в функцию передается копия переданных данных. Для обхода данного фишки можно использовать ключевое слово **inout**. И тогда функция сможет изменять переменную, но не забудь добавить знак         
      амперсанта **&** в передаваемых параметрах функции во время вызова.
      Сквозные параметры не могут иметь значения по умолчанию, а вариативные параметры не могут быть сквозными, с ключевым словом inout.
  
* ***Зачем нужен defer?***

    * defer нужен для выполнения набора инструкций перед тем как исполнение кода оставит текущий блок. Это позволяет сделать любую необходимую очистку. Он выполниться после return и перед выходом из области видимости. Пишется обычно в начале области видимости. 
      Если указаны несколько defer то они выполняться в обратном порядке, снизу вверх.

      
* ***Чем полезны extensions и какие возможности у них есть?***

  * Расширения (extension) позволяют добавить новую функциональность к уже существующему объектному типу (классу, структуре, перечислению) или протоколу. Таким образом вы можете расширять возможности любых типов, даже тех, что входят в состав Swift, то есть таких  
    типов данных, доступ к исходным кодам которых у вас отсутствует.
    Расширения могут добавлять новую функциональность к типу, но не могут изменять существующую. **Суть расширения состоит исключительно в наращивании возможностей, но не в их изменении.**
    Так же расширения могут добавить реализацию по умолчанию для Протоколов.

## Class Struct

* ***Можно ли не писать структуре инициализатор?***
    * Можно, у структуры встроенный init, который позволяет не писать его вручную.
  
* ***Что где хранится?***
    * Структура хранится в стеке, классы хранятся в куче!
      
* ***Что быстрее?***
    * Структуры быстрее, потому что они хранятся в стеке, что позволяет им иметь более быстрый доступ к данным. Так же структуры не требуют сбоки мусора из памяти, они сами уничтожаются после отработки. Еще структуры это типо значения, что означает создание новой копии при присвоении значения. По этой причине операции со структурами более предсказуемы и не требуют дополнительной синхранизации.
      
* ***В каких случаях что бы использовал?***
    * Если не нужно наследование и нужно скорость и безопасность, то нужно выбрать структуру. Вообще всегда нужно выбирать структуру, если только не нужны особености классов.
      
* ***Почему в структуре нет наследования?***
    Потому что структура это тип значения, а наследования работают с ссылками, то есть для этого нужно ссылочный тип как класс.

* ***Почему размер структуры должен быть известен заранее?***
    * Для производительности. Компилятор может оптимизировать если заранее знает размер структуры, что в свой очередь повышает производительность программы.
...
* ***Что такое композиция класса***
    ...
    * Композиция - это мощный инструмент, который позволяет создавать сложные системы из простых компонентов.
      
      Предположим, у нас есть два класса: Car (автомобиль) и Engine (двигатель).

      Мы можем создать композитный класс SportsCar (спортивный автомобиль), который:
      * Содержит объект класса Engine в качестве своего члена.
      * Делегирует объекту класса Engine ответственность за запуск, остановку и регулировку скорости.
      
## Stack vs Heap

* ***Что быстрее?***
    * Стек быстрее(для небольших данных, так как доступ идет к ним напрямую).
      
* ***Чем хорош стек? Для каких операций он идеален?***
    *  Алгоритм:
        * Стек: Подходит для алгоритмов LIFO (последний пришел - первый вышел), например, рекурсия.
        * Куча: Подходит для алгоритмов, где требуется произвольный доступ к данным, например, сортировка.
    * Потокобезопасность:
        * Стек: Каждый поток имеет свой стек, что обеспечивает потокобезопасность.
        * Куча: Куча используется совместно всеми потоками, что может привести к проблемам с потокобезопасностью.

* ***Какие типы где хранятся?***
    * Структура храниться в стеке, как и Перечисления, протоколы, обычные типы и тд.
    * Класс хранится в куче, как и Акторы и Замыкания

* ***Inderect enum - стек или куча?***
    ...
    * **indirect enums** обычно размещаются на куче (heap). **Косвенные перечисления** используются для создания рекурсивных структур данных, которые могут иметь переменную длину, что обычно подразумевает выделение памяти на куче во время выполнения программы.
      
* ***В теле функции создали переменную, где она будет храниться?***
     * Переменная, созданная в теле функции, будет храниться в стеке, если она не является статической.
       
            func myFunction() {
               let x = 10 // x хранится в стеке
          
               static var y = 20 // y хранится в куче
          
               y += 1
            }

        * x - локальная переменная, которая хранится в стеке. Она уничтожается после завершения myFunction.
        * y - статическая переменная, которая хранится в куче. Она сохраняет свое значение между вызовами myFunction.
  
* ***Случаи, когда value объекты будут в куче / reference в стэке***
* ***У нас есть функция которая принимает объект, что будет когда мы передадим обьект reference type и объект value type?***
    * Reference Type:
        * Ссылка на объект передается в функцию.
        * Изменения, внесенные в объект внутри функции, **будут видны извне**.
...
* ***В чем разница для компилятора в escaping closure и non-escaping?***
    * Для компилятора разница между escaping и non-escaping closures заключается в том, как он должен управлять памятью и жизненным циклом замыкания, чтобы обеспечить безопасность и эффективность выполнения кода.
        * Escaping Closures: Компилятор должен генерировать код, который сохраняет копию захваченных переменных до тех пор, пока замыкание не будет вызвано.
        * Non-Escaping Closures: Поскольку компилятор знает, что замыкание не может сохраниться для использования после завершения работы функции, он может принять меры для уменьшения накладных расходов на управление памятью.
    
* ***Как можно оптимизировать размер памяти, занимаемой структурой? (aligment)***
    * Использование типов данных с фиксированным размером (например, Int32 вместо Int)
    * Использование опциональных типов данных вместо значений по умолчанию
    * Размещение полей типа Int16 рядом друг с другом поможет избежать использования лишней памяти
