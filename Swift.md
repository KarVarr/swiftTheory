...
Если вам нужно объявить константу или переменную тем же именем, что и зарезервированное слово Swift, то вы можете воспользоваться обратными кавычками (`) написанными вокруг этого слова. let `var` = 44
print() По умолчанию функция заканчивает вывод символом переноса строки. Чтобы вывести в консоль значения без переноса на новую строку, вам нужно указать пустую строку в параметре terminator, например, print(someValue, terminator: "").
...
Коллекции 
Есть три вида коллекций, первый массив, второй сет, третий словари или дикшенери. Различия в них есть: 
У массива есть индекс и значение для поиска значения по индексу.
У сет есть ТОЛЬКО значения. 
У словаря есть ключ и значение. 
Так же в сет и в словаре нет постоянной последовательности, так как в них хранятся уникальные значения. В словаре не могут быть два одинаковых ключа так же как в сет не могут быть два одинаковых значения. У всех них есть различные методы. Они могут быть изменяемые и неизменяемые(let var). Они работают через дженерики<T>. 
Сет нельзя объявить упрошенным способом как массив или словарь, только через дженерик Set<Type>
...
Switch есть case где можно указать все нужные параметры через запятую a, b, c и так же у case есть временные переменные и дополнительная проверка с помощью слова where  case let (x, y) where x == y:
Еще есть операторы передачи управления     • continue break fallthrough return throw
...
Регулярные выражения в Swift - Regex
Регулярные выражения позволяют нам выполнять сложные операции поиска и замены в тысячах текстовых файлов всего за несколько секунд. Regex. По сути он помогает проверить на валидность нужное тебе выражение, почту, строку и и тд. Указываешь нужные параметры и готово. 
...
Вариативный параметры - Еще есть вариативный параметры(три точки) которые можно указать после типа параметра, что бы он принимал при вызове функции сразу много значений, и тогда это сразу будет массив… можно использовать только один параметр в данной функции или несколько, но тогда нужно внешнее имя для второго вариативного параметра ,то есть ярлык func arithmeticMean(int: Int..., sumForDouble numbers: Double...) -> Double {}.
Сквозные параметры - Параметры функции по умолчанию являются константами. Так как в функции нельзя изменять внешнюю переменную, ведь в функцию передается копия переданных данных, то для обхода данного фишки можно использовать ключевое слово inout перед типом. И тогда функция сможет изменять переменную, но не забудь добавить знак амперсанта & в передаваемых параметрах функции во время вызова.
...
Higher-order functions (Функции высшего порядка): Это функции, которые вы передаете в качестве аргументов другим функциям. В этом простом примере, фильтр требует передачи функции высшего порядка.
...
Минусы - наследование. Почему? Потому что глубокая иерархия наследованная, и если будет баг в середине этой цепочки, то непонятно куда идти, вверх или вниз по цепочке.
...
Если в кложуре много будет self, и произойдет то, что мы вышли из этого места выполнения кода, а он продолжился… для этого надо использовать Стронг ссылку guard self = self else { return }
...
Any AnyObject is as as? тайпкастинг 
...
CocoaPods SPM менеджер зависимостей
...
И viewWillTransitionToSize если нужно самостоятельно обработать анимацию при повороте, то можно его. 
    ...
    0.       init - установка свойств vc;
    0.       loadView - подмена стандартной view на кастомную;
    0.       viewDidLoad - view нашего ис создано, используется для заполнения UI;
    0.       viewWillAppear - vc начинает начинает появлять на экране, используется для обновления UI данными, которые могли измениться, пока ус не отображался на экране;
    0.       viewWilLayoutSubviews - вызывается перед тем как view нашего vc, разместит свои subviews. Границы окончательно подсчитаны.
    0.       view DidLayoutSubviews - уведомляет, что view нашего ус только что разместила свои subviews;
    0.       viewDidAppear - вызывается как только ис появился на экране;
    0.       viewWillDisappear - vc начинает удаляться с экрана;
    0.       viewDidDisappear - vc удален с экрана;
    0.        deinit - деинициализация vc, удаление из памяти устройства;
SwiftMagic
...
Спагетти-код (Spaghetti code) паттерн Единой ответсвенности. Solid
...
Для Обновление элемента есть метод SecItemUpdate.
...
Жизненный цикл у него такой:
    ...
    •    Меняем кадр
Render Server - это отдельный процесс, который вызывает методы отрисовки для GPU с использованием OpenGL или Metal. Он отвечает за рендер наших слоев в изображение
